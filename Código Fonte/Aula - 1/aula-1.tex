...
{\lstset{
		...
		morekeywords={usepackage,begin,end,documentclass,includegraphics, setmainfont, comando},
		#1
}}
{}
...
\title{Aula 1 - Tutorial de teX \hspace{1pt} básico}
...
\textbf{Alguns comandos básicos do LaTeX incluem:}

\begin{itemize}
	\item \texttt{\textbackslash\textbackslash} : cria uma quebra de linha dentro do texto.
	\item \texttt{\textbackslash\textbackslash[1em]} : adiciona um espaço vertical específico (exemplo: 1em) entre linhas.
	\item \texttt{\textbackslash newline} : também quebra a linha, similar a \texttt{\textbackslash\textbackslash}.
	\item \texttt{\textbackslash vspace\{valor\}} : insere espaço vertical do tamanho especificado.
	\item \texttt{\textbackslash hspace\{valor\}} : insere espaço horizontal do tamanho especificado.
	\item \texttt{\textbackslash textbf\{\}} : deixa o texto em negrito.
	\item \texttt{\textbackslash textit\{\}} : deixa o texto em itálico.
	\item \texttt{\textbackslash texttt\{\}} : deixa o texto em fonte monoespaçada (texto técnico).
	\item \texttt{\%} : inicia um comentário que será ignorado pelo compilador.
\end{itemize}
...
\section{Criando um Título:}

Para o nosso primeiro documento, é importante ter um título básico como mecanismo de identificação. O título permite que qualquer leitor identifique rapidamente o autor, a data de criação e o tema do documento. \\ 

No LaTeX, isso é feito preenchendo três macros principais no preâmbulo:
...
\subsection{A Macro \texttt{\textbackslash author}}

A macro \texttt{\textbackslash author\{\}} define o nome do autor do documento. É comum incluir apenas o nome completo, mas também é possível adicionar informações adicionais, como filiação institucional:

\begin{teX}
	\author{Vinícius Queiroz Fonseca \\ Centro Universitário Jorge Amado}
\end{teX}

\subsection{A Macro \texttt{\textbackslash date}}

A macro \texttt{\textbackslash date\{\}} define a data que aparecerá no título do documento. Caso você não queira especificar uma data, é possível deixar o campo em branco ou usar a macro \texttt{\textbackslash today} para inserir a data do dia da compilação automaticamente:

\begin{teX}
	\date{\today} % Insere a data atual
\end{teX}

\subsection{A Macro \texttt{\textbackslash title}}

A macro \texttt{\textbackslash title\{\}} define o título do documento. Tente manter o título claro e conciso, de preferência com poucas palavras, para facilitar a identificação do tema principal.

\begin{teX}
	\title{Aula 1 - Introdução ao LaTeX}
\end{teX}

\subsection{Gerando o Título no Documento}

Depois de definir essas macros no preâmbulo, o título é inserido no documento através do comando \texttt{\textbackslash maketitle}, que deve ser colocado logo após o início do ambiente \texttt{document}:
...
O LaTeX irá gerar automaticamente o título, mostrando o nome do autor, a data e o título conforme especificado, formatando-os de forma padronizada e profissional.
...
O teX{} fornece diversos símbolos matemáticos prontos para uso.
